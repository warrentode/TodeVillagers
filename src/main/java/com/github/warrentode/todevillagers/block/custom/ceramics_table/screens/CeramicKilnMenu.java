...
import com.github.warrentode.todevillagers.block.custom.ceramics_table.slots.CeramicKilnFuelSlot;
import com.github.warrentode.todevillagers.block.custom.ceramics_table.slots.CeramicKilnResultSlot;
...
public class CeramicKilnMenu extends RecipeBookMenu<RecipeWrapper> {
    public static final int FUEL_SLOT = 4;
    public static final int RESULT_SLOT = 5;
    public static final int INVENTORY_SIZE = RESULT_SLOT + 1;
    public final CeramicTableBlockEntity ceramicTableBlock;
    public final ItemStackHandler inventory;
    protected final Level level;
    private final ContainerData ceramicTableData;
    @SuppressWarnings("unused")
    private final ContainerLevelAccess canInteractWithCallable;

    public CeramicKilnMenu(int id, @NotNull Inventory playerInventory, BlockEntity blockEntity, ContainerData data) {
        super(ModMenuTypes.CERAMIC_KILN_MENU.get(), id);
        this.ceramicTableBlock = (CeramicTableBlockEntity) blockEntity;
        this.inventory = ((CeramicTableBlockEntity) blockEntity).getInventory();
        this.ceramicTableData = data;
        this.level = playerInventory.player.level();
        this.canInteractWithCallable = ContainerLevelAccess.create(Objects.requireNonNull(ceramicTableBlock.getLevel()), ceramicTableBlock.getBlockPos());
        /* this value must be the same as the one given for the size of the itemStackHandler within the CeramicTableBlockEntity */
        ...
        // Fuel Slot
        this.addSlot(new CeramicKilnFuelSlot(ceramicTableData, inventory, FUEL_SLOT, 62, 63));
        // Result Slot
        this.addSlot(new CeramicKilnResultSlot(playerInventory.player, ceramicTableBlock, inventory, RESULT_SLOT, 122, 45));
    ...
    public CeramicKilnMenu(int id, Inventory inventory, @NotNull FriendlyByteBuf extraData) {
        /* the size of the SimpleContainerData must be the same size the getCount within the CeramicTableBlockEntity container data
    ...
    private static @NotNull CeramicTableBlockEntity getBlockEntity(final Inventory playerInventory, final FriendlyByteBuf data) {
        Objects.requireNonNull(playerInventory, "playerInventory for Ceramic Kiln cannot be null");
        Objects.requireNonNull(data, "Ceramic Kiln data cannot be null");
        final BlockEntity blockAtPos = playerInventory.player.level().getBlockEntity(data.readBlockPos());
        if (blockAtPos instanceof CeramicTableBlockEntity) {
            return (CeramicTableBlockEntity) blockAtPos;
        }
        throw new IllegalStateException("Ceramic Kiln entity is not correct! " + blockAtPos);
    }

    @Override
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level, ceramicTableBlock.getBlockPos()),
                player, ModBlocks.CERAMICS_TABLE.get());
    }
    ...
    public boolean isCrafting() {
        return ceramicTableData.get(0) > 0;
    }

    public boolean hasFuel() {
        return ceramicTableData.get(2) > 0;
    }

    public int getScaledProgress() {
        // these are from the cases set in the matching block entity class file
        int progress = this.ceramicTableData.get(0);
        int maxProgress = this.ceramicTableData.get(1);
    ...
    public int getScaledFuelProgress() {
        // these are from the cases set in the matching block entity class file
        int fuelTime = this.ceramicTableData.get(2);
        int fuelDuration = this.ceramicTableData.get(3);
        // the pixel length or height of the progress arrow/bar in this GUI
        // if your arrow/bar is vertical, you need the height, otherwise you need the length
    ...
    public @NotNull RecipeBookType getRecipeBookType() {
        return ModRecipes.CERAMIC_BOOK;
    }
